...
\textbf{Determinate change (Definition~C):} A causal event produces a single-valued transition map $T_e$ (main paper, Eq.~\ref{eq:transition}). For the global dynamics $f$, this means $f$ is a function, and distinguishability of states requires $f$ to be injective: for any two states $s\neq s'$, $f(s)\neq f(s')$.
...
In heat conduction, the Cattaneo relaxation time $\tau$ is a material constant (temperature-dependent but externally specified). In the v3 $q$-field framework, the effective damping $\gamma_0(1-q)$ is \emph{derived from Definition~C}: occupied cells block new causal events, producing damping proportional to the occupied fraction. The damping is not a free parameter --- it is a dynamical consequence of the same capacity constraint that defines $q$.
...
M3 & \textbf{NEW:} Damping derivation was post-hoc, not from Definition~C & SEVERE (new) & RESOLVED-BY-PROOF \\
...
\item[\textbf{F1:}] \textbf{$\gamma$ was a free parameter.} \textit{Resolution:} Derived $\gamma(q)=\gamma_0(1-q)$ from Definition~C. Proof: causal events encounter occupied cells with probability $1-q$; each encounter blocks the current; effective damping $\propto (1-q)$. The linear ansatz $\gamma(q)=\gamma_0(1-q)$ is the simplest form; the essential property $\gamma\to 0$ as $q\to 1$ is independent of the specific functional form. See \S A3.1.
