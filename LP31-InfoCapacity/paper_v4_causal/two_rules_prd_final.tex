...
\title{Gravity and Quantum Decoherence from Causal Structure}
...
\begin{abstract}
We propose that both gravitational attraction and quantum decoherence are consequences of a single mechanism: the bounded information capacity of physical cells, derived from a single definition---a causal event is an asymmetric influence producing a determinate change---plus a small set of structural choices enumerated in Table~I.
...
The damping $\gamma(q)=\gamma_0(1-q)$ is the simplest ansatz consistent with the causal definition: occupied cells block new causal events.
...
\textbf{Caveat on Zilly's theorem.}
Zilly's theorem is a recent result (2026) and, like any new theoretical development, awaits independent verification and community scrutiny.
The mathematical derivation of Eqs.~(\ref{eq:telegraph_unified})--(\ref{eq:metric}) from Definition~C and the Tier~2 structural choices does not invoke Zilly's theorem. However, the identification of the cell spacing $a$ with the Planck length $\ell_P$ relies on $\hbar$, which enters the framework through Zilly's theorem; and the physical interpretation of the 1-bit capacity bound as connected to quantum coherence depends on Zilly's theorem as the quantum-mechanical foundation. The classical $q$-field equations are self-contained; their quantum interpretation is not.
Zilly's theorem provides the quantum-mechanical context but is not load-bearing for the derivation of the unified telegraph equation, dynamical Lorentz selection, or Newtonian gravity.
...
Asymmetry provides the arrow of time. Determinate change---formalized as a single-valued transition map $T_e: s \mapsto T_e(s)$ on the cell's state space---implies, via three lemmas and a theorem (Sec.~\ref{sec:axioms}.A), that every elementary causal event transfers exactly 1 bit. The 1-bit quantization is not an additional postulate; it follows from requiring that the transition map be single-valued (determinate), non-identity (something changes), and elementary (not decomposable). Finite propagation at speed $c$ entails locality. Between causal events, a cell's state is objectively undetermined---this undeterminedness \emph{is} quantum superposition, whose mathematical form is provided by Zilly's theorem.

The full proof of Theorem~1 (1-bit quantization), an honest inventory of the assumptions challenged by each lemma, and the division of labor between Definition~C and Zilly's theorem are given in Sec.~\ref{sec:axioms}.A.
...
The derivation of the unified telegraph equation, the dynamical Lorentz selection, the static Newtonian limit, and the H-theorem require only Definition~C and the Tier~2 structural choices enumerated in Table~\ref{tab:tiers}.
...
Section~\ref{sec:gamma} derives $\gamma(q)=\gamma_0(1-q)$ from the causal definition and frames $\gamma_0$ through naturalness.
...
This definition contains only two primitive concepts---\emph{asymmetry} and \emph{determinate change}---plus the discrete cell as the locus of the event. Everything else follows. The remainder of this subsection unpacks the definition and shows that the 1-bit quantization, locality, and quantum superposition are not additional assumptions layered on top of it; they are consequences of what the definition asserts.
...
\begin{itemize}
    ...
    \item \textbf{From Zilly's theorem:} Complex Hilbert space structure, the Born rule, unitary dynamics, and the full mathematical apparatus of quantum mechanics.
\end{itemize}
The two chains converge at the 1-bit cell: Definition~C establishes that causal events are 1-bit events; Zilly's theorem establishes that a finite-capacity system whose states are distinguished by 1-bit events necessarily obeys quantum mechanics. The convergence is nontrivial---Definition~C and Zilly's theorem were developed independently, yet both point to the same irreducible unit---but it is not a logical identity. The bridge between the classical causal graph and the quantum Hilbert space is Zilly's theorem, and we rely on it as an external input.
...
\begin{enumerate}
    ...
    \item \textbf{Superposition} (undeterminedness between causal events $\Rightarrow$ quantum coherence, with mathematical form from Zilly's theorem).
\end{enumerate}
...
But the architecture of our argument does not crumble if the theorem requires revision---the classical gravitational limit derived in Sections~\ref{sec:static}--\ref{sec:selforg} depends only on Definition~C and the Tier~2 structural choices, not on the specific mechanism by which quantum mechanics emerges.
...
In the continuum limit $a\to0$, the denominator $(1-\bar{q}_{ij})$ approaches a common value across the neighborhood of $i$, and to leading order factors out of the sum. A cleaner path (detailed in the SM, Sec.~A3.5) derives the same result directly from the static limit of the telegraph equation, $\partial_t^2 q + \gamma_0(1-q)\partial_t q = c^2\nabla^2(\ln q)$, by setting all time derivatives to zero, which immediately yields $\nabla^2(\ln q)=0$ without factor-cancellation arguments. Either path gives the same static equation. The condition reduces to:
...
The proper distance traversed by a signal is $dl_{\rm proper} = dl_{\rm coord}/q$, yielding $g_{rr} = 1/q^2$ in isotropic coordinates. (Note: a simple mean-hop-time argument gives an effective propagation speed $c_{\rm eff} = c q$, which would imply $dr/dt = c q$ for a radial light ray. The full metric derived below gives $dr/dt = c q^2$ from $ds^2=0$. The discrepancy is a factor of $q$ and signals that the mean-hop-time argument is a heuristic; the metric relation $g_{rr}=1/q^2$ is adopted as the simplest isotropic form consistent with the Newtonian limit and PPN parameters. A rigorous derivation of $g_{rr}$ from cell-graph signal propagation remains an open problem.)
...
One definition, plus the Tier~1--2 inputs enumerated in Table~\ref{tab:tiers}. The results, in compressed form:
...
\begin{enumerate}
    ...
    Their connection to quantum mechanics relies on Zilly's (2026) theorem as an external input, not on Definition~C alone.
    Within the classical field description, the results are self-contained: the unified telegraph equation, dynamical Lorentz selection, and Newtonian gravity are derived from the $q$-field equations without quantum interpretation.
    The quantum interpretation of $q$ as a coherence measure requires the cell-basis mapping established via the $\ell_1$-norm equivalence (Theorem~T4 in the Supplemental Material); a basis-independent quantum interpretation remains Open Problem~O5.
...
We thank J.~Zilly for providing the foundational proof that finite information capacity plus self-referential consistency yields quantum mechanics~\cite{Zilly:2026}, without which this work would lack its definitional grounding.
