...
This SM provides the complete mathematical scaffolding behind every claim in the main paper. No result in the main text is asserted without a traceable derivation here.

The v3 paper defines the $q$-field framework on $N$ binary cells $\{s_i\in\{0,1\}\}_{i=1}^N$, capacity bounded by A2. The vacant capacity fraction is $q=N^{-1}\sum_i(1-s_i)$. The continuum limit is taken as $N\to\infty$, $a\to0$, giving a scalar field $q(\mathbf{x},t)$ on $\mathbb{R}^3$.
...
\midrule
IGM & $10^{-7}$ & $\approx 1$ & $3.1{\times}10^{34}$ & $2.0{\times}10^{-34}$ & $1.5{\times}10^{42}$ \\
...
\textbf{Consistency with GW170817.} The v3 framework predicts $v_{\rm GW} = c$ in vacuum to within one part in $10^{70}$ (non-dispersive static shift) or $10^{80}$ (dispersive correction)---both astronomically below the $10^{-15}$ constraint. However, as noted in the main paper (Sec.~8.3), this analysis covers scalar $q$-field modes; the observed GW170817 signal may correspond to tensor metric perturbations whose decoupling from the scalar sector is conjectured but not yet derived. Full consistency therefore requires proof of tensor-scalar decoupling at linear order, which remains an open problem.
...
This section catalogues every issue found by v1, v2, and v3 adversarial reviews, with resolution status. It serves as the paper's immune system: nothing is hidden.
...
Of the 13 new issues found by the v2 to v3 adversarial review, all are resolved. F1 ($\gamma$ was a free parameter) was the most severe: it was resolved by deriving the capacity-dependent damping $\gamma(q)=\gamma_0(1-q)$ from A2---the central new result of v3. F2 and F3 (no Lorentz emergence argument) fell when we proved the damping term is second-order in $\delta q$ around $q=1$, making Lorentz invariance an exact emergent symmetry. The narrative arc of this section is simple: v1 had fundamental problems, v2 repaired some, v3 fixed the deepest structural issues. Four problems remain open and deserve attention in any future extension.
...
\item[\textbf{F1:}] \textbf{$\gamma$ was a free parameter.} \textit{Resolution:} Derived $\gamma(q)=\gamma_0(1-q)$ from A2 (capacity bound). Proof: information flow encounters occupied cells with probability $1-q$; each encounter blocks the current; effective damping $\propto (1-q)$. The linear ansatz $\gamma(q)=\gamma_0(1-q)$ is the simplest form; the essential property $\gamma\to 0$ as $q\to 1$ is independent of the specific functional form. See \S A3.1.

\item[\textbf{F2:}] \textbf{No preferred-frame containment.} \textit{Resolution:} Proved that $\gamma(q)=0$ exactly at $q=1$ (vacuum). Preferred-frame effects, which require $\gamma>0$, are therefore confined to regions with $q<1$, i.e., near mass. The suppression factor is $(1-q) \sim GM/rc^2 \sim 10^{-9}$ at the solar surface. See \S A3.4.

\item[\textbf{F3:}] \textbf{Lorentz emergence not rigorous.} \textit{Resolution:} Proved that damping $\gamma_0(1-q)\partial_t q$ is $\mathcal{O}(\delta q^2)$ around $q=1$, so the linearized dynamics are exactly $\Box(\delta q)=0$ --- a Lorentz-invariant wave equation. Lorentz invariance is not ``assumed'' or ``emerges in a limit''; it is the exact linearization. See \S A3.4.
...
\item[\textbf{F5:}] \textbf{Telegraph equation had constant $\gamma$.} \textit{Resolution:} The v3 telegraph equation has capacity-dependent damping $\gamma_0(1-q)$. The constant-$\gamma$ case is recovered only as the leading approximation when $q$ is nearly uniform; the full nonlinear equation includes the $(1-q)$ factor. See \S A3.3.
...
A complete mapping between SM notation, main paper notation, and deprecated v1/v2 notation. Nothing should be ambiguous when cross-referencing.
...
\textbf{Critical rule:} The coupling constant is $\kappa\equiv c^2/a^2$ (used interchangeably in the main paper's H-theorem section). The damping is $\gamma_0(1-q)$, NOT $1/\tau$ or constant $\gamma$. Any SM equation using $\tau$ or constant $\gamma$ is referencing deprecated v1/v2 notation and is inconsistent with the v3 paper.
