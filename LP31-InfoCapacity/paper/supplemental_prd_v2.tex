...
The v3 paper defines the $q$-field framework on a set of $N$ binary cells $\{s_i\in\{0,1\}\}_{i=1}^N$, with capacity bounded by A2. The vacant capacity fraction is $q=N^{-1}\sum_i(1-s_i)$. In the continuum limit $N\to\infty$ with cell spacing $a\to0$, $q$ becomes a scalar field $q(\mathbf{x},t)$ on $\mathbb{R}^3$.

\textbf{Unified current equation (v3):}
\begin{equation}
...
\textbf{Unified telegraph equation (v3):}
\begin{equation}
...
All continuum conclusions are reliable in the regime $a|\nabla\ln q| \ll 1$. Every equation is numbered for traceability, and all approximations are explicitly flagged. This SM is the paper's strongest defense against referee criticism --- every claim in the main text is traced to its mathematical origin here.
...
The v3 paper's fundamental current dynamics on edge $(i,j)$ is:
\begin{equation}
\partial_t J_{ij} = -\frac{c^2}{a^2}(\ln q_i - \ln q_j) - \gamma_0(1-\bar{q})J_{ij},
\label{eq:a3_current}
\end{equation}
...
\textbf{No separate ``A3 axiom'' is needed.} Finite propagation speed is part of A2 (the speed bound $c$). The Cattaneo-type current relaxation follows from the microscopic picture: information carriers move with speed $c$ between cells; the current has inertia because carriers cannot change direction instantaneously.
...
The sign deserves care. In the discrete equation, $-(c^2/a^2)(\ln q_i - \ln q_j)$ drives $J_{ij}$ toward the direction of decreasing $\ln q$. If $\ln q_i < \ln q_j$, then $-(\ln q_i-\ln q_j) > 0$, driving $J_{ij}$ positive (flow from $i$ to $j$, from lower $q$ to higher $q$). In the continuum, $\nabla(\ln q)$ points toward increasing $\ln q$, so a positive coefficient $+c^2$ drives $\mathbf{J}$ in the direction of $\nabla(\ln q)$, which is from lower to higher $\ln q$ --- consistent.
...
This is the fundamental dynamical equation of the v3 framework. It is nonlinear through both the $\ln q$ term and the $(1-q)$ damping factor.
...
\textbf{Comparison with earlier (v1/v2) formulation:} In earlier versions, the telegraph equation was $\partial_t^2 q + \gamma\partial_t q = (\kappa/\tau)\nabla^2(\ln q)$ with signal speed $c_s = \sqrt{\kappa/(\tau q_0)}$, which required calibration $\kappa/\tau = c^2 q_0$ to match observations. In the v3 unified formulation, $c$ appears directly as the propagation speed, the damping is $\gamma_0(1-q)$ rather than a constant $\gamma$, and no ad hoc calibration of the wave speed is needed.
...
For the parabolic $q$-field equation $\partial_t q = \kappa\nabla^2(\ln q)$, the free information functional $\mathcal{F}[q] = \int \Phi(q)\,dx$ with $\Phi(q)=q\ln q - q + 1$ is a Lyapunov functional (proved in v1 SM). For the unified telegraph equation (\ref{eq:a3_telegraph}), we require a Lyapunov functional that accounts for the kinetic energy of the current $\mathbf{J}$ and the capacity-dependent damping $\gamma_0(1-q)$. This section proves that such a functional exists for both the full nonlinear and linearized cases --- a \textbf{genuine new result} of the v3 framework.
...
\item The result generalizes the constant-$\gamma$ Cattaneo $H$-theorem (v1 SM, \S A7.5) to the physically motivated $\gamma(q)=\gamma_0(1-q)$ damping, and is a \textbf{new result}.
\end{enumerate}
...
For $q_0\to 1$ (vacuum): $k_{\rm cross} \to 0$. \textbf{All modes are in the wave regime near vacuum.} For $q_0\to 0$ (near saturation): $k_{\rm cross} \to \gamma_0/(2c) \sim 1.7\times 10^{34}$~m$^{-1}$ --- the crossover enters the Planck regime.
...
\textbf{Key improvement over v1 SM:} The singular perturbation is now expressed in terms of the physically meaningful $\varepsilon = \omega/[\gamma_0(1-q)]$ rather than the formal limit $\gamma\to\infty$, which obscured the fact that near vacuum the system is \emph{not} overdamped at any frequency.
...
\textbf{CORRECTION from SM v1:} The earlier estimate $N\sim 10^{90}$ used a nuclear-scale cell size $\sim 10^{-15}$~m, which is the scale of quantum field theory degrees of freedom, not the DGF cell scale. The DGF cell spacing is identified with the Planck length $\ell_P$ (see \S F3). The correct estimate is:
...
This is a fiducial estimate. The actual number is not derived from A1-A3 and may differ by many orders of magnitude. However, as long as $N \gg 10^{20}$, the continuum approximation is effectively exact for all macroscopic purposes.
...
\subsubsection{C1.4 Key difference from the v3 unified framework}

In heat conduction, the Cattaneo relaxation time $\tau$ is a material constant (temperature-dependent but externally specified). In the v3 $q$-field framework, the effective damping $\gamma_0(1-q)$ is \emph{derived from the capacity bound A2}: occupied cells block information flow, producing damping proportional to the occupied fraction. The damping is not a free parameter --- it is a dynamical consequence of the same capacity constraint that defines $q$.
...
The v3 $q$-field framework makes the identical conceptual move: the current $\mathbf{J}$ is elevated from a constitutive relation to a dynamical field obeying its own equation of motion (\ref{eq:a3_continuum_current}). The crucial difference is that in the London theory, the ``inertia'' parameter $\Lambda$ is a material property; in the v3 framework, the corresponding inertia is fixed by the fundamental speed $c$ and cell spacing $a$, with the damping $\gamma_0(1-q)$ following from the capacity bound.
...
For a lossless line ($R=G=0$), this is the wave equation. With damping, it is the telegrapher's equation. The v3 telegraph equation (\ref{eq:a3_telegraph}) has the same structure, with the crucial difference that the damping coefficient $\gamma_0(1-q)$ depends on the field $q$ itself --- a nonlinear transmission line analogy.
...
\textbf{Preamble:} The unified telegraph equation (\ref{eq:a3_telegraph}) is hyperbolic and describes finite-speed propagation of $q$-field perturbations. All predictions in this section follow from the unified current equation (\ref{eq:a3_current}) and its continuum limit. Unlike v1/v2 SM, there is no separate ``hyperbolic extension'' to postulate --- the telegraph equation is the fundamental dynamics. The subsections D2-D5 are new for v3.
...
\midrule
Intergalactic medium & $10^{-7}$ & $\approx 1$ & $3.1\times 10^{34}$ & $2.0\times 10^{-34}$ & $1.5\times 10^{42}$ \\
Solar surface & $2\times 10^{-6}$ & $\approx 1$ & $6.2\times 10^{35}$ & $1.0\times 10^{-35}$ & $3.0\times 10^{43}$ \\
Earth surface & $7\times 10^{-10}$ & $\approx 1$ & $2.2\times 10^{32}$ & $2.9\times 10^{-32}$ & $1.0\times 10^{40}$ \\
Laboratory & $10^{-27}$ & $\approx 1$ & $3.1\times 10^{14}$ & $2.0\times 10^{-14}$ & $1.5\times 10^{22}$ \\
Near BH ($r=2r_s$) & $0.393$ & $0.607$ & $2.8\times 10^{42}$ & $2.2\times 10^{-42}$ & $1.4\times 10^{50}$ \\
Planck scale & $---$ & $0$ & $9.3\times 10^{33}$ & $6.8\times 10^{-34}$ & $4.4\times 10^{41}$ \\
\bottomrule
\end{tabular}
\end{table}
...
GW170817 constrained $|v_{\rm GW} - c|/c \le 10^{-15}$. We compute the integrated phase velocity correction through the Milky Way halo to determine whether the v3 framework satisfies this bound.
...
\begin{equation}
1 - q_0(R_\odot^{\rm gal}) \approx \frac{GM_{\rm halo}}{R_\odot^{\rm gal} c^2} \sim \frac{6.67\times 10^{-11}\times 10^{12}\times 2\times 10^{30}}{8\times 3.09\times 10^{19}\times 9\times 10^{16}} \approx 6\times 10^{-7}.
\end{equation}
...
\begin{equation}
\frac{\Delta v_\phi}{c} \sim \frac{1}{2}\int_{\rm LOS} (1-q_0(\mathbf{x}))\,\frac{d\ell}{D} \sim \frac{GM_{\rm MW}}{2D c^2} \sim 10^{-70},
...
\begin{equation}
\int_{\rm MW} (1-q_0(\ell))\,d\ell \approx \frac{GM_{\rm MW}}{c^2}\ln\frac{R_{\rm halo}}{R_{\rm min}} \sim \frac{GM_{\rm MW}}{c^2}\times \mathcal{O}(10) \sim 10^{23}~\text{m}.
\end{equation}

With $\omega \sim 2\pi\times 100$~Hz $\sim 600$~s$^{-1}$:
\begin{equation}
\Delta\phi \sim \frac{600}{2\times 3\times 10^8}\times 10^{23} \sim 10^{17}~\text{radians}.
\end{equation}
...
\begin{equation}
\frac{\Delta v_\phi}{c}\bigg|_{\rm disp} \sim \left(\frac{k}{k_{\rm cross}}\right)^2 \sim 10^{-80}.
\end{equation}

\subsubsection{D4.5 Conclusion}

\textbf{GW170817 is trivially satisfied.} The v3 framework predicts $v_{\rm GW} = c$ in vacuum to within one part in $10^{70}$ (non-dispersive static shift) or $10^{80}$ (dispersive correction). Both are astronomically below the $10^{-15}$ constraint. No fine-tuning is required --- the suppression follows from the same $GM/rc^2$ factor that makes gravity weak.
...
\textbf{Flag:} All predictions in D1-D5 depend on the unified telegraph equation (\ref{eq:a3_telegraph}) and the coupling to matter via (\ref{eq:scalar_wave}). Unlike v1/v2 SM, there are no ``scalar GW polarization'' predictions (retracted in v2). The v3 framework's sole clean GW prediction is the vacuum wave speed $c$, which is exact and trivially consistent with GW170817.
...
\begin{table}[h]
\caption{Complete audit issue inventory with v2$\to$v3 resolution status.\\
RESOLVED-BY-PROOF = mathematical proof provided; RESOLVED-BY-REMOVAL = claim/feature removed; RESOLVED-BY-REWRITE = section rewritten; OPEN = under investigation.}
...
F1 & \textbf{NEW:} $\gamma$ was a free parameter, not derived & FATAL (new) & RESOLVED-BY-PROOF \\
F2 & \textbf{NEW:} No preferred-frame containment argument & SEVERE (new) & RESOLVED-BY-PROOF \\
F3 & \textbf{NEW:} Lorentz emergence not rigorous & SEVERE (new) & RESOLVED-BY-PROOF \\
F4 & \textbf{NEW:} Wave speed $c_s$ required separate calibration & SEVERE (new) & RESOLVED-BY-REWRITE \\
F5 & \textbf{NEW:} Telegraph equation had constant $\gamma$, inconsistent with capacity & SEVERE (new) & RESOLVED-BY-PROOF \\
F6 & \textbf{NEW:} Lyapunov functional only for constant $\gamma$ & MODERATE (new) & RESOLVED-BY-PROOF \\
...
M1 & \textbf{NEW:} Audit trail in v1/v2 SM was self-congratulatory & MODERATE (new) & RESOLVED-BY-REWRITE \\
M2 & \textbf{NEW:} Scalar GW polarization (D2-D4) not retracted clearly & SEVERE (new) & RESOLVED-BY-REMOVAL \\
M3 & \textbf{NEW:} Damping derivation was post-hoc, not from A2 & SEVERE (new) & RESOLVED-BY-PROOF \\
M4 & \textbf{NEW:} $c_s$ calibration was an independent postulate & MODERATE (new) & RESOLVED-BY-REWRITE \\
M5 & \textbf{NEW:} No proof that boundary terms vanish in Lyapunov & MINOR (new) & RESOLVED-BY-PROOF \\
...
\textbf{V2 $\to$ V3 malicious reviewer findings (F1-F7, M1-M6):} These 13 new issues were identified by the v2$\to$v3 adversarial review and are all RESOLVED. The most severe finding (F1: $\gamma$ was a free parameter) was resolved by deriving $\gamma(q)=\gamma_0(1-q)$ from A2, which is the central new result of v3. The second most severe (F2/F3: no Lorentz emergence argument) was resolved by proving that damping is second-order in $\delta q$ around $q=1$, making Lorentz invariance an exact emergent symmetry.

% ============================================================
\subsection{E2. Resolution Details for New (v3) Issues}
% ============================================================

\begin{enumerate}[leftmargin=*, label=\textbf{F1:}]
\item \textbf{$\gamma$ was a free parameter.} \textit{Resolution:} Derived $\gamma(q)=\gamma_0(1-q)$ from A2 (capacity bound). Proof: information flow encounters occupied cells with probability $1-q$; each encounter blocks the current; effective damping $\propto (1-q)$. The linear ansatz $\gamma(q)=\gamma_0(1-q)$ is the simplest form; the essential property $\gamma\to 0$ as $q\to 1$ is independent of the specific functional form. See \S A3.1.

\item \textbf{F2: No preferred-frame containment.} \textit{Resolution:} Proved that $\gamma(q)=0$ exactly at $q=1$ (vacuum). Preferred-frame effects, which require $\gamma>0$, are therefore confined to regions with $q<1$, i.e., near mass. The suppression factor is $(1-q) \sim GM/rc^2 \sim 10^{-9}$ at the solar surface. See \S A3.4.

\item \textbf{F3: Lorentz emergence not rigorous.} \textit{Resolution:} Proved that damping $\gamma_0(1-q)\partial_t q$ is $\mathcal{O}(\delta q^2)$ around $q=1$, so the linearized dynamics are exactly $\Box(\delta q)=0$ --- a Lorentz-invariant wave equation. Lorentz invariance is not ``assumed'' or ``emerges in a limit''; it is the exact linearization. See \S A3.4.

\item \textbf{F4: Wave speed required calibration.} \textit{Resolution:} In the unified framework, the wave speed is $c$ by construction (from the $c^2/a^2$ coupling in the current equation). No separate calibration $\kappa/\tau = c^2 q_0$ is needed. See \S A3.7.

\item \textbf{F5: Telegraph equation had constant $\gamma$.} \textit{Resolution:} The v3 telegraph equation has capacity-dependent damping $\gamma_0(1-q)$. The constant-$\gamma$ case is recovered only as the leading approximation when $q$ is nearly uniform; the full nonlinear equation includes the $(1-q)$ factor. See \S A3.3.

\item \textbf{F6: Lyapunov functional only for constant $\gamma$.} \textit{Resolution:} Proved $dE/dt \le 0$ for the generalized energy functional with capacity-dependent damping $\gamma_0(1-q)$. This is a new result. See \S A7.
...
\item \textbf{Born rule.} Not derived from A1-A3. \textbf{Status: Not addressed.} (Zilly's theorem provides the Born rule from finite capacity plus cyclic dynamics; we accept it as input.)
...
\textbf{Honest assessment:} Of the 10 open problems, OP-3 (coupling to matter) is the most urgent for empirical contact beyond the static Newtonian correspondence. OP-1 (cell basis) is the deepest conceptual issue. The remaining problems, while important, are less immediately blocking for the framework's coherence.
...
The static calibration requires that the source term in $\nabla^2\psi = -(4\pi G/c^2)\rho_m$ reproduces Newtonian gravity. In the DGF framework, the coupling does not involve $\gamma_0$ directly (it enters through the static limit of the current equation). The dimensional consistency check from v1 SM \S F1.2 carries over: $\ell_P c = 4.8\times 10^{-27}$~m$^2$/s vs.\ $4\pi G/c^2 = 9.3\times 10^{-27}$~m$^2$/s, differing by a factor of $\sim 2$ that is absorbed into an $\mathcal{O}(1)$ dimensionless coupling.
...
\textbf{Intergalactic medium} ($1-q \sim 10^{-7}$): $k_{\rm cross} \sim 3\times 10^{34}$~m$^{-1}$, $\lambda_{\rm cross} \sim 2\times 10^{-34}$~m $\sim 12\,\ell_P$.

\textbf{Solar surface} ($1-q \sim 2\times 10^{-6}$): $k_{\rm cross} \sim 6\times 10^{35}$~m$^{-1}$, $\lambda_{\rm cross} \sim 1\times 10^{-35}$~m $\sim 0.6\,\ell_P$.

\textbf{Laboratory} ($1-q \sim 10^{-27}$): $k_{\rm cross} \sim 3\times 10^{14}$~m$^{-1}$, $\lambda_{\rm cross} \sim 2\times 10^{-14}$~m (nuclear scale).
...
This section provides a complete mapping between the notation used in this SM and the v3 main paper, plus a mapping from the deprecated v1/v2 notation.

\begin{table}[h]
\caption{Notation mapping: v3 paper $\leftrightarrow$ this SM $\leftrightarrow$ deprecated v1/v2.}
\label{tab:notation}
\begin{tabular}{llll}
\toprule
Quantity & v3 Paper \& This SM & Deprecated v1/v2 & Notes \\
\midrule
...
Effective damping & $\gamma_0(1-q)$ & $\gamma$ (constant) & key v3 innovation \\
...
\textbf{Critical rule:} The coupling constant in the current equation is $c^2/a^2$, NOT $\kappa$. The damping is $\gamma_0(1-q)$, NOT $1/\tau$ or constant $\gamma$. Any SM equation using $\kappa$, $\tau$, or constant $\gamma$ is referencing deprecated v1/v2 notation and is inconsistent with the v3 paper.
