...
For small perturbations $\delta q$ around $q=1$, $\ln q\approx q-1$ recovers the linear telegraph equation; the static limit gives $\nabla^2(\ln q)=0$ directly, without patching.
...
In vacuum ($q\to1$), $\gamma\to0$ and Lorentz invariance is \textbf{dynamically selected} as the ground state of the empty universe---preferred-frame effects are confined to matter-filled regions.
...
Our starting point is Zilly's (2026) theorem~\cite{Zilly:2026}: from finite information capacity and self-referential consistency (``graded equality''), the full mathematical structure of quantum mechanics---complex amplitudes, the Born rule, and unitary dynamics---emerges as the unique consistent description.
This result is foundational: it means quantum mechanics is itself a consequence of bounded capacity, not a separate axiom.
Zilly's theorem also establishes $\hbar$ as the natural scale of the capacity bound, fixing the quantization of phase space volume~\cite{Zilly:2026}.
We accept Zilly's theorem as the quantum-mechanical foundation and build upward toward gravity.
...
The theory requires exactly \textbf{one structural choice}---the dynamical variable $\phi(q)=-\ln q$ in the universality class $\Phi$ of monotonic functions diverging at $q\to0$---and the cell spacing $a$ as the single new dimensional parameter, identified with the Planck length $\ell_P\equiv\sqrt{\hbar G/c^3}$ through a naturalness argument (the theory contains no new scales beyond $c$, $\hbar$, $G$).
...
We note that Zilly's theorem is a recent result (2026) and, like any new theoretical development, awaits independent verification and community scrutiny.
Should it require revision, the classical gravitational limit derived in Sections~\ref{sec:static}--\ref{sec:selforg} depends only on A1--A2 and the Tier~2 structural choices, not on the specific mechanism by which quantum mechanics emerges.
...
We classify all inputs to the theory into four tiers, distinguishing irreducible physical principles from modeling choices.
...
With $N\sim10^{184}$ Planck cells in the observable universe and $N_b\sim10^{60}$ cells per mesoscopic block (e.g., at the nuclear scale), continuum error estimates are $\mathcal{O}(10^{-30})$ (see SM~B5).
...
For $q_i,q_j\approx1$, $\ln q_i-\ln q_j\approx q_i-q_j$, recovering the linear approximation.
But as $q\to0$, the logarithmic gradient diverges, encoding the capacity bound's resistance to further inflow.

The damping term must also respect the capacity bound: occupied cells block information flow, so the effective damping seen by a current $J_{ij}$ should scale with the average occupation of the two cells involved.
...
It appears once, in this form, and is used consistently throughout the paper.
...
Differentiating Eq.~\eqref{eq:continuity} with respect to time and substituting Eq.~\eqref{eq:current_fundamental}:
...
It is derived once, from the unified current equation~\eqref{eq:current_fundamental}, and used consistently throughout.
...
The linearized dynamics are \textbf{exactly} the wave equation---the damping term, being second order in the perturbation, does not appear at linear order.
...
However, a physically important regime occurs when the inertial term is negligible compared to damping:
\begin{equation}
...
Damping is not an external friction parameter but an \emph{intrinsic} consequence of the capacity bound (A2).
...
What fixes $a$? The theory contains exactly three dimensionful constants from Tier~1: $c$, $\hbar$, and $G$.
...
This is a \textbf{naturalness argument}, not a logical necessity.
...
Conversely, if predictions made with $\gamma_0=c/\ell_P$ are confirmed, the naturalness argument is supported.
...
Only quantitative values ($k_{\rm cross}$ magnitude, absolute damping timescales) scale with $\alpha$.
...
Substituting $\gamma(q)=\gamma_0(1-q)$ into the unified telegraph equation~\eqref{eq:telegraph_unified}:
...
This is the nonlinear telegraph equation with capacity-dependent damping, identical to Eq.~\eqref{eq:telegraph_unified}.
It is reproduced here to emphasize that the same equation governs all regimes---no separate ``linearized'' or ``nonlinear'' versions are needed.
...
From the fundamental current equation~\eqref{eq:current_fundamental} with $\partial_t J_{ij}=0$:
...
The derivation is \textbf{one continuous argument} from Eq.~\eqref{eq:current_fundamental} to Eq.~\eqref{eq:laplace_ln}:
...
Spatial dimensionality $d=3$ is taken as an empirical input (Tier~1), but it plays a distinguished role in the theory.
...
\subsection{Source Term: Occupied Cells as Information Charge}
\label{sec:source}

The static equation $\nabla^2(\ln 1/q)=0$ describes vacuum configurations sourced by a boundary condition at infinity (the integration constant $GM/c^2$).
A complete theory must include how occupied cells source the $q$-field locally.

In the presence of occupied cells---which store information and therefore reduce local free capacity---the static equation generalizes to:

\begin{equation}
\boxed{\nabla^2(\ln 1/q) = \frac{4\pi G_{\rm eff}}{c^2}\,\frac{1-q}{a^3}}.
\label{eq:poisson_source}
\end{equation}

The right-hand side is proportional to the occupied-capacity fraction $(1-q)$ per cell volume $a^3$, representing the information charge density.
This is the framework's expression of the equivalence principle: \textbf{mass IS locked information}.
The identification of occupied capacity fraction with mass density,
\begin{equation}
\rho_m \propto \frac{1-q}{a^3},
\end{equation}
is the natural bridge between information theory and gravitational phenomenology.
The coupling constant $G_{\rm eff}$ is calibrated to match Newton's constant $G$ in the nonrelativistic, weak-field limit.

In the vacuum ($q\to1$), the source term vanishes and Eq.~\eqref{eq:poisson_source} reduces to $\nabla^2(\ln 1/q)=0$, consistent with the static derivation above.
For small $1-q$, the exponential solution~\eqref{eq:q_solution} satisfies the homogeneous equation and provides the leading-order exterior solution around an isolated mass.
The full Poisson-type equation~\eqref{eq:poisson_source} would govern the interior structure and the transition from the Newtonian to the strong-field regime.
...
where $\kappa\equiv c^2/a^2$ and $\Phi(q)$ satisfies $\Phi'(q)=\ln q$.
...
The energy $E$ is bounded below (by zero, since $\Phi(q)\geq0$ and $|\mathbf{J}|^2\geq0$), so the dynamics monotonically drive the system toward an energy minimum.
...
The first term in $E$, $\int\Phi(q)\,d^3x$, is the configuration entropy of the $q$-field.
The second term, $\int|\mathbf{J}|^2/(2\kappa)\,d^3x$, is the kinetic energy of information currents.
The damping $\gamma_0(1-q)$ dissipates this kinetic energy at a rate proportional to the local occupation fraction, driving the system toward the entropy-maximizing static configuration.
...
It is essential to distinguish predictions that depend on the specific choice $\phi(q)=-\ln q$ from those protected by universality.
...
\begin{itemize}
    ...
    \item The specific form of the source term in Eq.~\eqref{eq:poisson_source}.
...
\begin{equation}
\boxed{k_{\rm cross} = \frac{\gamma_0(1-q_0)}{2c}}.
\label{eq:kcross}
\end{equation}
...
Following the classification of Sec.~\ref{sec:qual_vs_quant}, we separate the predictions:
...
All assumptions are classified into four tiers (Table~\ref{tab:tiers}): Tier~0--1 are irreducible inputs, Tier~2 choices are protected by universality, and Tier~3 quantities are fixed by Tiers~0--2 plus naturalness.
The cell spacing $a$ is the single new dimensional parameter; identifying $a$ with $\ell_P$ follows from the requirement that the theory introduce no new scales beyond $c$, $\hbar$, $G$.
...
A significant open issue in the $q$-field framework is the tension with the holographic principle.
The capacity bound of 1 bit per cell is a \textbf{local} bound applied to each cell independently.
For a region of space of size $R$, the total information capacity is proportional to the number of cells in that region, scaling as the volume:
...
This is not a minor discrepancy---it is a qualitative difference in the scaling of the maximum information content of spacetime.

\textbf{Why this matters.}
The holographic principle is supported by two independent lines of evidence:
(1) black hole thermodynamics (the Bekenstein--Hawking entropy $S_{\rm BH}=A/4\ell_P^2$ is proportional to horizon area, not volume), and
(2) the AdS/CFT correspondence, in which a gravitational theory in $d+1$ dimensions is equivalent to a nongravitational theory in $d$ dimensions, explicitly realizing area-law entanglement entropy.
...
We identify three directions, none of which is currently realized in the framework:

\begin{enumerate}
    \item \textbf{Nonlocal cell connectivity.} The 1-bit-per-cell bound counts independent degrees of freedom. If cells inside a region are \emph{not} all independently accessible---because quantum entanglement or gravitational nonlocality reduces the effective number of independent degrees of freedom---then the naive volume count overestimates the true information capacity. Concretely, if cells within a region of size $R$ are pairwise entangled at the Planck scale, the von Neumann entropy of the reduced state of the region scales with the number of entangled pairs crossing the boundary, which is $\propto R^2/a^2$---automatically yielding area scaling. The holographic bound may thus emerge as an entanglement entropy bound, with the 1-bit-per-cell capacity serving as the ultraviolet cutoff rather than the infrared scaling law.

    \item \textbf{Holographic cell arrangement.} The regular 3D lattice (Tier~2 modeling choice) may be the wrong discretization. A holographic arrangement of cells on the boundary of each region, with bulk cells being non-fundamental (determined by boundary data), would automatically yield area scaling. This would require replacing the nearest-neighbor 3D lattice with a boundary-cell graph.

    \item \textbf{Saturation not achieved.} The 1-bit bound is an \textbf{ultraviolet} bound per cell. Whether all cells within a volume can simultaneously saturate this bound depends on the dynamics. If gravitational collapse enforces a tighter bound---the Bekenstein bound $S\leq 2\pi RE/(\hbar c)$---then the volume scaling $R^3$ is never realized in practice, and the effective capacity is limited by the area-scaling holographic bound.
\end{enumerate}

\textbf{Honest assessment.}
The holographic tension is a serious limitation of the current framework.
The 1-bit-per-cell bound is a natural consequence of A2, but it gives volume scaling, not area scaling.
Resolving this tension---showing how the holographic bound emerges from the $q$-field dynamics, or modifying the cell structure to incorporate holography---is a priority for future work.
We do not claim to have resolved this tension; we flag it prominently as an open problem whose solution may point toward the extension to full general relativity.

\subsection{Fair Comparison with HTMAU Using Tiered Methodology}
\label{sec:htmau_comparison}

Bassani and Magueijo (2025)~\cite{Bassani:2025htma} proposed ``How to Make a Universe'' (HTMAU), a framework in which diffeomorphism invariance emerges from a Markov chain of stochastic choices evolving toward an absorbing state.
Both HTMAU and the present $q$-field theory share the philosophy that spacetime symmetries are emergent rather than fundamental.
However, the inputs, mechanisms, and outputs differ substantially.
To make the comparison fair and systematic, we apply the \textbf{same tiered methodology} (Sec.~\ref{sec:tiered}) to both theories.

\subsubsection{HTMAU's Assumptions in Tiered Classification}

We enumerate HTMAU's specific assumptions, as presented in Ref.~\cite{Bassani:2025htma}:
...
If the former, then HTMAU's richer structure is necessary, and the present theory must be extended to recover diffeomorphism invariance---a direction we discuss in the open directions below.

We emphasize that this comparison is methodological, not adversarial.
Both approaches contribute to the broader program of understanding spacetime as emergent.
The tiered methodology provides a systematic framework for comparing the economy and explanatory power of different approaches.
...
\begin{enumerate}
    ...
    \item \textbf{Matter coupling is not fully derived.} We assumed spherical symmetry and identified an integration constant with $GM/c^2$. A complete theory must derive how $T_{\mu\nu}$ sources $q$ in general configurations. The source term proposed in Eq.~\eqref{eq:poisson_source} is a first step but requires rigorous derivation from the microscopic dynamics.
...
\begin{enumerate}
    \item \textbf{Full GR recovery.} The most important open problem is extending the $q$-field framework to recover the full Einstein equations. The nonlinear structure of $\nabla^2(\ln 1/q)=0$ hints at a deeper connection to the Ricci tensor, but the mapping is not yet known. The source term~\eqref{eq:poisson_source} suggests a Poisson-type equation for $\ln q$, which may relate to the $G_{00}$ component of the Einstein tensor in the Newtonian gauge.
