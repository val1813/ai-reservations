...
def cosmic_decoherence_order() -> None:
    q_access = np.linspace(0, 1, 401)
    archive = 1.0 - q_access
    mass = (2.0 / np.pi) * np.sin(np.pi * archive / 2.0)
    classicality = 1.0 - np.exp(-8.0 * archive**2)

    write_csv(
        DATA_DIR / "cosmic_decoherence_order.csv",
        ["q_access", "archive_fraction_A", "mass_over_mp", "classicality_proxy"],
        [
            [float(qi), float(ai), float(mi), float(ci)]
            for qi, ai, mi, ci in zip(q_access, archive, mass, classicality)
        ],
    )

    fig, ax1 = plt.subplots(figsize=(5.2, 3.4))
    ax1.plot(q_access, mass, color="#1f77b4", lw=2, label=r"$m(A)/m_p$")
    ax1.plot(q_access, archive, color="#b22222", lw=2, label=r"archived information $A=1-q$")
    ax1.set_xlabel("accessible coherence q")
    ax1.set_ylabel("normalized scale")
    ax1.set_ylim(0, 1.05)
    ax2 = ax1.twinx()
    ax2.plot(q_access, classicality, color="#2ca02c", lw=2, ls="--", label="classicality proxy")
    ...
    ax1.set_title("DGF archive-decompression order")
    fig.tight_layout()
