...
\Gamma(q)\ge0 .
\end{equation}
There is no global pairing between heavy and light systems. Information disperses locally, but low-mass receiver modes have larger unfilled capacity per Planck cell and statistically absorb more flowing coherence---a statistical capacity-gradient effect analogous to heat flow without designated molecular partners. The drift is set by receiver state-space multiplicity: $v(q)=\Gamma(q)\partial_qS_{\rm recv}(q)>0$, where $S_{\rm recv}=k_B\ln\mathcal{N}_{\rm recv}(q)$ counts receiver states. This is heuristically analogous to spontaneous emission (transition rate set by density of final states, a receiver-side quantity), though the precise mathematical objects differ. For low-mass radiationlike receivers, $\partial_qS_{\rm recv}>0$ and $v(q)>0$. The reflecting boundaries at $q=0,1$ are effective; near $q=1$, the Fokker-Planck description gives way to the absorbing-emitting boundary of the archive-decompress transition (Supplemental Material S3.3, S11).
...
\begin{equation}
\boxed{
m_{\max}=\frac{2m_p}{\pi}\simeq1.385\times10^{-8}{\rm kg}\;(\approx14\,\mu\text{g}).
}
\end{equation}
\textbf{Per-branch Euler mass, not rest mass.} $m(q)$ is the Euler residual mass---the mass-equivalent of the information-geometric deficit---not the total rest mass $m_{\rm rest}$. Rest mass is given by total information $m_{\rm rest}=(m_p/\eta)(I_{\rm locked}+I_{\rm flowable})$ and is constant during decoherence (Supplemental Material S10.3). The Euler mass $m(q)$ parameterizes the coupling to receiver modes via $V_S=p^4/3m_{\rm rest}$; the endpoint $m_{\max}=2m_p/\pi\approx14\,\mu\text{g}$ is the maximum Euler residual before spatial coherence fragments. A macroscopic object consists of many such fragments, each with $q\approx1$, but their rest masses sum to $M_{\rm rest}\gg m_{\max}$. Falsifiable prediction: a single coherent system maintaining full interference access above $2m_p/\pi$ would refute DGF.
...
\end{equation}
For concrete receivers: a photon bath gives super-ohmic $J_{\rm recv}^{\rm(photon)}(\omega)\propto\hbar\omega^3$ (dipole coupling); a graviton bath gives $J_{\rm recv}^{\rm(grav)}(\omega)\sim t_p^2\hbar\omega^5 m_S^2R_S^4$ (quadrupole coupling). Both have UV cutoff at $\omega_c=t_p^{-1}$. The receiver mass appears only through coupling strength in $J_{\rm recv}$, never in the denominator ($V_S=p^4/3m_{\rm rest}$ belongs to the emitter, with $m_{\rm rest}$ the constant rest mass). In the effective Planck-cell limit, the kernel coefficient $2t_pl_p^4/\hbar^6$ is set by dimensional analysis and is universal at the current level of UV completion; different receiver sectors are distinguished by their non-Markovian corrections (Supplemental Material S7).
\begin{equation}
\boxed{
K_{ab}(t)=
\exp\left[
